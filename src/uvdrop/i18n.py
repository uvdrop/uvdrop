...
_STRINGS: dict[str, dict[str, str]] = {
    ...
    "app.hibernate": {"ja": "環境を冬眠", "en": "Hibernate env", "zh": "休眠环境"},
    "app.delete": {"ja": "削除", "en": "Delete", "zh": "删除"},
    "app.refresh": {"ja": "更新", "en": "Refresh", "zh": "刷新"},
    "app.mode_ready": {"ja": "準備済み", "en": "Ready", "zh": "已就绪"},
    "app.mode_hibernated": {"ja": "冬眠中", "en": "Hibernated", "zh": "休眠中"},
    ...
    "settings.tab_storage": {"ja": "ストレージ", "en": "Storage", "zh": "存储"},
    ...
    "settings.hibernate_title": {
        "ja": "仮想環境の冬眠",
        "en": "Virtual environment hibernation",
        "zh": "虚拟环境休眠",
    },
    "settings.hibernate_help": {
        "ja": "アプリ一覧・作業フォルダ・.env・ショートカットは残し、使っていない venv だけを削除します。次回起動時は通常のガード確認後に uv sync で再構築します。",
        "en": "Keep the app entry, workspace, .env, and shortcut; remove only an unused venv. The next launch rebuilds it with uv sync after the normal guards.",
        "zh": "保留应用条目、工作区、.env 和快捷方式，仅删除未使用的 venv。下次启动会在正常防护检查后通过 uv sync 重建。",
    },
    "settings.hibernate_enable": {
        "ja": "起動時に、長期間使っていない venv を冬眠する（既定OFF）",
        "en": "At startup, hibernate venvs unused for a long time (off by default)",
        "zh": "启动时休眠长期未使用的 venv（默认关闭）",
    },
    "settings.hibernate_after": {
        "ja": "最終起動から",
        "en": "After",
        "zh": "距上次启动",
    },
    "settings.hibernate_days": {"ja": "日で冬眠", "en": "days", "zh": "天后休眠"},
    "settings.hibernate_note": {
        "ja": "ON にした場合だけ、uvdrop 起動時に判定します。uv のグローバルキャッシュは削除しないため、再構築時のダウンロードを再利用できます。アプリ一覧の「環境を冬眠」ならいつでも手動実行できます。",
        "en": "Checked only at uvdrop startup when enabled. The uv global cache is kept, so downloads can be reused when rebuilding. Use “Hibernate env” in the app list for manual hibernation anytime.",
        "zh": "仅在启用后于 uvdrop 启动时检查。不会删除 uv 全局缓存，因此重建时可复用下载。也可随时在应用列表中手动“休眠环境”。",
    },
    "settings.hibernate_days_invalid": {
        "ja": "日数は 1〜3650 の整数で指定してください。",
        "en": "Days must be an integer from 1 to 3650.",
        "zh": "天数必须是 1 到 3650 的整数。",
    },
    ...
    "hibernate.confirm": {
        "ja": "「{name}」の仮想環境だけを冬眠しますか？\nアプリ一覧・作業フォルダ・.env・ショートカットは残ります。\n次回起動時に uv sync で再構築します。",
        "en": "Hibernate only the virtual environment for “{name}”?\nThe app entry, workspace, .env, and shortcut remain.\nIt will be rebuilt with uv sync on next launch.",
        "zh": "仅休眠“{name}”的虚拟环境吗？\n应用条目、工作区、.env 和快捷方式会保留。\n下次启动时将通过 uv sync 重建。",
    },
    "hibernate.done": {
        "ja": "「{name}」を冬眠しました（約 {size} 解放）。",
        "en": "Hibernated “{name}” (about {size} reclaimed).",
        "zh": "已休眠“{name}”（约释放 {size}）。",
    },
    "hibernate.already": {
        "ja": "「{name}」はすでに冬眠中です。",
        "en": "“{name}” is already hibernated.",
        "zh": "“{name}”已处于休眠状态。",
    },
    "hibernate.failed": {
        "ja": "仮想環境を冬眠できませんでした。アプリが実行中なら終了してから再試行してください。\n{e}",
        "en": "Could not hibernate the virtual environment. If the app is running, close it and try again.\n{e}",
        "zh": "无法休眠虚拟环境。如果应用正在运行，请关闭后重试。\n{e}",
    },
    "hibernate.gc_log": {
        "ja": "未使用の仮想環境を {n} 件冬眠しました: {keys}",
        "en": "Hibernated {n} inactive virtual environment(s): {keys}",
        "zh": "已休眠 {n} 个未使用的虚拟环境：{keys}",
    },
    "licenses.missing": {
